...
<script lang="ts">
    ...
    `);
		...
		cleanups.push(() => window.removeEventListener('mousedown', onMouseDown));
		cleanups.push(() => document.body.removeEventListener('mousemove', handleFirstMouseMove));
		cleanups.push(() => window.removeEventListener('mousemove', onMouseMove));
		cleanups.push(() => document.body.removeEventListener('touchstart', handleFirstTouchStart));
