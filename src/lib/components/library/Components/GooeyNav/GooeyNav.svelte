	...
	<nav class="flex relative" style="transform:translate3d(0,0,0.01px);">
		...
		>
			{#each items as item, index (index)}
				<li class="rounded-full relative cursor-pointer transition-[background-color_color_box-shadow] duration-300 ease shadow-[0_0_0.5px_1.5px_transparent] text-white {activeIndex === index ? 'active' : ''}">
					<a
						href={item.href}
...
<style>
	...
	:global(.effect.text.active) { color: black; }
	...
	:global(.effect.active::after) { animation: pill 0.3s ease both; }
	@keyframes pill { to { transform: scale(1); opacity: 1; } }
	:global(.particle), :global(.point) { display: block; opacity: 0; width: 20px; height: 20px; border-radius: 9999px; transform-origin: center; }
	:global(.particle) { --time: 5s; position: absolute; top: calc(50% - 8px); left: calc(50% - 8px); animation: particle calc(var(--time)) ease 1 -350ms; }
	:global(.point) { background: var(--color); opacity: 1; animation: point calc(var(--time)) ease 1 -350ms; }
	@keyframes particle {
