...
<script module lang="ts">
	export type MasonryItem = { id: string; img: string; url: string; height: number };
</script>
...
<div bind:this={containerRef} class="relative w-full h-full">
	{#each grid as item (item.id)}
		...
		>
			...
			>
				{#if colorShiftOnHover}
					<div class="color-overlay absolute inset-0 rounded-[10px] bg-gradient-to-tr from-pink-500/50 to-sky-500/50 opacity-0 pointer-events-none"></div>
				{/if}
			</div>
		</div>
	{/each}
