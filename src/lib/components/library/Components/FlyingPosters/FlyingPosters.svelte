...
`;
	...
	const loadImageElement = loadWebGLImage;
	...
	class Media {
		...
		createShader() {
			...
			const img = loadImageElement(this.image);
			img.onload = () => {
				texture.image = img;
			...
			img.onerror = () => {
				console.warn(
					`[FlyingPosters] Image blocked (often CORS): ${this.image}. Use same-origin files or a host that sends Access-Control-Allow-Origin.`
				);
			};
		}
	...
	class Canvas {
		...
		createPreloader() {
			...
			this.items.forEach((src) => {
				const image = loadImageElement(src);
				const markLoaded = () => {
