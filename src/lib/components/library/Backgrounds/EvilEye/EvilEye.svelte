<!-- @svelte-bits {"title":"Evil Eye","description":"Reactive evil eye background with cursor-tracked pupil.","dependencies":["ogl"]} -->
<script lang="ts">
	...
	const current = $derived({ eyeColor, intensity, pupilSize, irisWidth, glowIntensity, scale, noiseScale, pupilFollow, flameSpeed, backgroundColor });
...
void main() {
  ...
  gl_FragColor = vec4(color, max(max(color.r, color.g), color.b));
		...
		const update = (time: number) => {
			...
			program.uniforms.uEyeColor.value = hexToVec3(current.eyeColor);
