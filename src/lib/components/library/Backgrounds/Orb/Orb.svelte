<!-- @svelte-bits {"title":"Orb","description":"Dreamy hue-shifting orb with hover interaction.","dependencies":["ogl"]} -->
<script lang="ts">
	...
	const current = $derived({ hue, hoverIntensity, rotateOnHover, forceHoverState, backgroundColor });
...
void main() {
  ...
  gl_FragColor = vec4(col.rgb * col.a, col.a);
		...
		const resize = () => {
			...
			renderer.setSize(containerRef.clientWidth * dpr, containerRef.clientHeight * dpr);
			gl.canvas.style.width = containerRef.clientWidth + 'px';
			gl.canvas.style.height = containerRef.clientHeight + 'px';
		...
		const update = (t: number) => {
			...
			const effective = current.forceHoverState ? 1 : targetHover;
			program.uniforms.hover.value += (effective - program.uniforms.hover.value) * 0.1;
			if (current.rotateOnHover && effective > 0.5) currentRot += dt * rotationSpeed;
			program.uniforms.rot.value = currentRot;
			program.uniforms.backgroundColor.value = hexToVec3(current.backgroundColor);
